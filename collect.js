...
var db = new sqlite3.Database('pooq.db');

var loadPage = function(name, id, patterns) {
    browser = new Browser();
    subBr= new Browser();

    var url = util.format("http://torrentgun.net/bbs/bbs.php?bo_table=torrent_%s&wr_id=%d", name, id);
    console.log(url);

    browser.visit(url, function() {
        // Check if this page exist
        var isBreak = false;
        browser.queryAll("legend").map(function(node) {
            if(browser.text(node) == '알림') isBreak = true;
        });
        if(isBreak) return;
        ...
        // Check next page
        loadPage(name, id+1, patterns);
    });
}

var loadPageSequencially = function(runSets) {
    console.log('##');
    runSet = runSets.pop();
    if(typeof runSet == 'undefined') return;

    sequence
        .then(function() {
            loadPage(runSet[0], runSet[1], runSet[2]);
        })
        .then(function() {
            loadPageSequencially(runSets);
        });
}
...
db.serialize(function() {
    ...
    db.each("SELECT * FROM category", 
        function(err, row) {
            ...
            var patterns = [];
            db.each(util.format("SELECT * FROM %sPatterns", row.name),
                function(err, row) {
                    patterns.push(new RegExp(row));
                }, function() {
                    runSet.push([name, id, patterns]);
                });
        },
        function() { 
            loadPageSequencially(runSet);
        });
});
